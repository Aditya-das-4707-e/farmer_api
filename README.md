...
Example: `GET /products/?name=rice` → market price(s) for rice varieties.
...
| Language | Python 3.12 (3.8+ should work) |
...
411 entries in `RAW_PRODUCTS` (`main.py:28-440`), each `(name, hindi, bengali, price, unit)`. Indicative general market rates in INR — **not live mandi prices**.
...
- Prices are hardcoded; to update, edit `RAW_PRODUCTS` in `main.py` and restart.
- Production run: `uvicorn main:app --host 0.0.0.0 --port 8000` (add `--workers 4` as needed, or put behind nginx/Docker).

## Limitations

- Static prices — no live update, no date/region/mandi breakdown.
...
- Live price ingestion / per-mandi rates.
